...
def main(args):
  ...
  if args.question is not None and args.image is not None:
    ...
    # exec_params = sum(p.numel() for p in model[1].parameters())
...
def run_single_example(args, model):
  ...
  #img = imread(args.image, mode='RGB') #old
  #img = imresize(img, img_size, interp='bicubic') #old
  img = imread(args.image, pilmode='RGB')
  img = np.array(Image.fromarray(img).resize((img_size), Image.BICUBIC))
  # img = cv2.resize(img, img_size, cv2.INTER_CUBIC) #interp='bicubic')
  ...
  predicted_answer = vocab['answer_idx_to_token'][predicted_answer_idx.item()]#[0]]
